<?php
	...
	// Dans le cas de l'appel Ajax, on force l'appel à la fonction afficherTableau
	// Pour déterminer le mode d'appel de la page, on regarde la présence d'un paramètre nommée appalAjax
	...
	function afficherTableau($bdd, $nomDivTableau, $numeroMatchMin, $numeroMatchMax, $nomDiv, $classeEquipe, $classeEspacement, $classeEspacementVertical, $finale) {
				...
				for($j = 0; $j < 2; $j++) {
					...
					*/
					...
					if($tableau[$indiceRecalcule]["Pronostiquable"] == 1)
						echo '<div class="' . $classeEquipe . ' ' . $alignement . '" onclick="creerProno_changerScoreMatch(' . $tableau[$indiceRecalcule]["Matches_Match"] . /*', ' . $tableau[$indiceRecalcule]["Ordre"] .*/ ', \'' . $nomDivTableau . '\');">';
					else
	...
	function afficherPodium($bdd, $tableau, $numeroMatchMin, $numeroMatchMax, $nomDiv) {
	...
	function afficherTableaux($bdd, $tableau) {
		afficherTableau($bdd, $tableau, 1, 8, 'divTableau1_8', 'equipe1_8', 'espacement1_8', 'espacementVertical1_8', 0);
		afficherTableau($bdd, $tableau, 9, 12, 'divTableau1_4', 'equipe1_4', 'espacement1_4', 'espacementVertical1_4', 0);
		afficherTableau($bdd, $tableau, 13, 14, 'divTableau1_2', 'equipe1_2', 'espacement1_2', 'espacementVertical1_2', 0);
		afficherTableau($bdd, $tableau, 15, 16, 'divTableau1_1', 'equipe1_1', '', 'espacementVertical1_1', 1);
		afficherPodium($bdd, $tableau, 15, 16, 'divPodium');
	}
	
	function afficherTableauxAppelAjax($bdd, $tableau) {
		afficherTableaux($bdd, $tableau);
	}
	
	if($appelAjax == 1) {
		afficherTableauxAppelAjax($bdd, $tableau);
	}
