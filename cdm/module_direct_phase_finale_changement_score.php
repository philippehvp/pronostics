<?php
	...
	function afficherScoreEquipe($unMatch, $equipeAB) {
		...
		echo '<select id="selectScoreEquipe' . $equipeAB . '" onchange="module_directPhaseFinale_sauvegarderScoreMatch(this, \'score\', ' . $unMatch["Matches_Match"] . ', \'' . $equipeAB . '\');" id="selectButs' . $equipeAB . '_match_' . $unMatch["Matches_Match"] . '"' . $disabled . '>';
			for($i = -1; $i <= 15; $i++) {
	...
	function afficherScoreAPEquipe($unMatch, $equipeAB) {
		...
		echo '<span id="spanProlongation' . $equipeAB . '_match_' . $unMatch["Matches_Match"] . '"' . $style . '>';
			echo '<select id="selectScoreAPEquipe' . $equipeAB . '" onchange="module_directPhaseFinale_sauvegarderScoreMatch(this, \'scoreAP\', ' . $unMatch["Matches_Match"] . ', \'' . $equipeAB . '\');" id="selectButsAP' . $equipeAB . '_match_' . $unMatch["Matches_Match"] . '"' . $disabled . '>';
	...
	function afficherTAB($unMatch) {
		...
		$pronostics_Vainqueur = $unMatch["Pronostics_Vainqueur"] != null ? $unMatch["Pronostics_Vainqueur"] : -1;
		...
		echo '<span id="spanVainqueur_match_' . $unMatch["Matches_Match"] . '"' . $style . '>';
			echo '<select id="selectVainqueur" onchange="module_directPhaseFinale_sauvegarderScoreMatch(this, \'vainqueur\',' . $unMatch["Matches_Match"] . ', \'V\');" id="selectVainqueur_match_' . $unMatch["Matches_Match"] . '"' . $disabled . '>';
				$selected0 = $pronostics_Vainqueur == -1 ? ' selected="selected"' : '';
				$selected1 = $pronostics_Vainqueur == 0 ? ' selected="selected"' : '';
				$selected2 = $pronostics_Vainqueur == $unMatch["EquipeA"] ? ' selected="selected"' : '';
				$selected3 = $pronostics_Vainqueur == $unMatch["EquipeB"] ? ' selected="selected"' : '';
				echo '<option value="-1"' . $selected0 . '>Encore en prolongation</option>';
				echo '<option value="0"' . $selected1 . '>Tirs au but</option>';

				echo '<option value="' . $unMatch["EquipeA"] . '"' . $selected2 . '>' . $unMatch["EquipeA_Nom"] . '</option>';
				
				echo '<option value="' . $unMatch["EquipeB"] . '"' . $selected3 . '>' . $unMatch["EquipeB_Nom"] . '</option>';
			echo '</select>';
		echo '</span>';
	}
