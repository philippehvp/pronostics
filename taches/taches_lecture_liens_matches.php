<?php
	...
	foreach($matches as $unMatch) {
		...
		$baliseCompoDomicile = $xpath->query('//td[@class="compo1"]');
		if($baliseCompoDomicile->length == 0) {
			continue;
		}
		
		$joueurs = $baliseCompoDomicile->item(0)->childNodes;
		$i = 0;
		foreach($joueurs as $unJoueur) {
			if($unJoueur->textContent != null) {
				$retour = ajouterJoueur($bdd, trim($unJoueur->textContent), $equipeDomicile, $unMatch["Match"], $dateMatch, 1);
				if($retour == -1)			ajouterErreur($bdd, $unMatch["Match"], 'Domicile : joueur ' . trim($unJoueur->textContent) . ' inconnu', 0);
				else if($retour == 0)		ajouterErreur($bdd, $unMatch["Match"], 'Domicile : joueur ' . trim($unJoueur->textContent) . ' trouvé mais plusieurs fois', 0);
		...
		$baliseCompoVisiteur = $xpath->query('//td[@class="compo2"]');
		if($baliseCompoVisiteur->length == 0) {
			continue;
		}
		
		$joueurs = $baliseCompoVisiteur->item(0)->childNodes;
		$i = 0;
		foreach($joueurs as $unJoueur) {
			if($unJoueur->textContent != null) {
				$retour = ajouterJoueur($bdd, trim($unJoueur->textContent), $equipeVisiteur, $unMatch["Match"], $dateMatch, 1);
				if($retour == -1)			ajouterErreur($bdd, $unMatch["Match"], ('Visiteur : joueur ' . trim($unJoueur->textContent) . ' inconnu'), 0);
				else if($retour == 0)		ajouterErreur($bdd, $unMatch["Match"], ('Visiteur : joueur ' . trim($unJoueur->textContent) . ' trouvé mais plusieurs fois'), 0);
